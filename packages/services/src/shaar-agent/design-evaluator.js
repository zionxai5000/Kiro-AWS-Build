...
exports.DesignEvaluator = void 0;
class DesignEvaluator {
    ...
    evaluate(observation) {
        ...
        const overallDesignScore = Math.round(Object.values(categoryScores).reduce((sum, s) => sum + s, 0) / Object.keys(categoryScores).length);
    ...
    evaluateTypography(observation) {
        const issues = [];
        const { html } = observation;
    ...
    evaluateCTAs(observation) {
        ...
        if (buttons.length > 5) {
            const allSameWeight = buttons.every(b => b.classes.join(' ') === buttons[0].classes.join(' '));
            if (allSameWeight) {
                issues.push({
                    id: 'cta-no-hierarchy',
                    severity: 'medium',
    ...
    evaluateNavigation(observation) {
        const issues = [];
        const { elements, html } = observation;
    ...
    evaluateConsistency(observation) {
        const issues = [];
        const { elements } = observation;
...
exports.DesignEvaluator = DesignEvaluator;
//# sourceMappingURL=design-evaluator.js.map