...
exports.DependencyGraphEngineImpl = void 0;
const node_crypto_1 = require("node:crypto");
// ---------------------------------------------------------------------------
// Implementation
// ---------------------------------------------------------------------------
class DependencyGraphEngineImpl {
     ...
     */
    async createGraph(tasks) {
        const dagId = (0, node_crypto_1.randomUUID)();
    ...
    async schedule(dag) {
        ...
        while (scheduled.size < taskIds.length) {
            ...
            const estimatedDuration = Math.max(...readyIds.map((id) => dag.tasks.get(id).estimatedDuration));
            batches.push({
                index: batchIndex,
                taskIds: readyIds,
                estimatedDuration,
        ...
        const estimatedTotalDuration = batches.reduce((sum, batch) => sum + batch.estimatedDuration, 0);
        return {
            dagId: dag.id,
            batches,
            estimatedTotalDuration,
    ...
    async detectDeadlocks(dagId) {
        ...
        for (const taskId of permanentlyBlocked) {
            const task = dag.tasks.get(taskId);
            const blockedByFailed = task.dependencies.filter((depId) => failedTaskIds.has(depId) || permanentlyBlocked.has(depId));
            blockedTasks.push({
                taskId,
    ...
    detectCycles(dag) {
        ...
        for (const edge of dag.edges) {
            ...
            if (!dag.tasks.has(edge.from) || !dag.tasks.has(edge.to))
                continue;
     ...
     */
    traceCycle(remainingNodes, adjacency) {
     ...
     */
    estimateCriticalPath(tasks, edges) {
        const taskMap = new Map();
        for (const task of tasks) {
            taskMap.set(task.id, task);
        }
     ...
     */
    findTransitivelyBlocked(dag, stateMap, failedTaskIds) {
        const blocked = new Set();
        const visited = new Set();
        const isBlocked = (taskId) => {
            if (visited.has(taskId))
                return blocked.has(taskId);
            visited.add(taskId);
...
exports.DependencyGraphEngineImpl = DependencyGraphEngineImpl;
