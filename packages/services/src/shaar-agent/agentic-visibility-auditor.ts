...
export interface AgenticVisibilityReport {
  ...
  isChatbotLike: boolean; // True if agent appears as simple chatbot
...
export class AgenticVisibilityAuditor {
  ...
  private checkAgenticFeatures(observation: PageObservation): AgenticFeatureCheck[] {
    const { html } = observation;
    const features: AgenticFeatureCheck[] = [];
  ...
  private generateIssues(features: AgenticFeatureCheck[], observation: PageObservation): AgenticVisibilityIssue[] {
    const issues: AgenticVisibilityIssue[] = [];
    ...
    for (const feature of features) {
      ...
      const categoryMap: Record<string, AgenticCategory> = {
  ...
  private detectChatbotAppearance(observation: PageObservation, features: AgenticFeatureCheck[]): boolean {
    const { html } = observation;
    ...
    const presentCount = features.filter(f => f.present).length;
    const criticalPresent = features.filter(f =>
      f.present && ['Execution Trace', 'Memory Indicators', 'Tool Usage Display'].includes(f.feature)
    ).length;
