...
exports.PolymarketDriver = exports.POLYMARKET_ERROR_CODES = void 0;
const driver_js_1 = require("../base/driver.js");
// ---------------------------------------------------------------------------
// Error Codes
// ---------------------------------------------------------------------------
exports.POLYMARKET_ERROR_CODES = {
...
// Polymarket Driver
// ---------------------------------------------------------------------------
class PolymarketDriver extends driver_js_1.BaseDriver {
    credentialManager;
    name = 'polymarket';
    version = '1.0.0';
    _apiKey = null;
    ...
    async doExecute(operation) {
        ...
        if (!this._driverConfig) {
            return this.errorResult(operationId, exports.POLYMARKET_ERROR_CODES.UNAUTHORIZED, 'Driver is not connected', false);
        }
        ...
        switch (operation.type) {
            ...
            default:
                return this.errorResult(operationId, exports.POLYMARKET_ERROR_CODES.UNSUPPORTED_OPERATION, `Unsupported operation type: ${operation.type}`, false);
        }
    }
    ...
    async handleGetMarkets(operation, operationId) {
        const { status, category, limit, cursor } = operation.params;
        const result = {
            success: true,
            data: {
                markets: [],
    ...
    async handlePlaceTrade(operation, operationId) {
        const { marketId, outcome, shares, price, type } = operation.params;
        // --- Parameter validation ---
        if (!marketId) {
            return this.errorResult(operationId, exports.POLYMARKET_ERROR_CODES.INVALID_PARAMS, 'marketId is required for placeTrade', false);
        }
        if (!outcome || !['Yes', 'No'].includes(outcome)) {
            return this.errorResult(operationId, exports.POLYMARKET_ERROR_CODES.INVALID_PARAMS, 'outcome must be "Yes" or "No"', false);
        }
        if (!shares || shares <= 0) {
            return this.errorResult(operationId, exports.POLYMARKET_ERROR_CODES.INVALID_PARAMS, 'shares must be a positive number', false);
        }
        if (price !== undefined && (price < 0.01 || price > 0.99)) {
            return this.errorResult(operationId, exports.POLYMARKET_ERROR_CODES.INVALID_PARAMS, 'price must be between 0.01 and 0.99', false);
        }
        ...
        if (newPosition > this._driverConfig.maxPositionSize) {
            return this.errorResult(operationId, exports.POLYMARKET_ERROR_CODES.POSITION_LIMIT_EXCEEDED, `Position size ${newPosition} would exceed limit of ${this._driverConfig.maxPositionSize} for market ${marketId}`, false, {
                currentPosition,
        ...
        if (this._riskState.dailyRealizedLoss + potentialLoss > this._driverConfig.dailyLossLimitUsd) {
            return this.errorResult(operationId, exports.POLYMARKET_ERROR_CODES.DAILY_LOSS_LIMIT_EXCEEDED, `Trade would risk exceeding daily loss limit of $${this._driverConfig.dailyLossLimitUsd}`, false, {
    ...
    async handleCancelTrade(operation, operationId) {
        ...
        if (!tradeId) {
            return this.errorResult(operationId, exports.POLYMARKET_ERROR_CODES.INVALID_PARAMS, 'tradeId is required for cancelTrade', false);
        }
        const result = {
            success: true,
            data: {
...
exports.PolymarketDriver = PolymarketDriver;
//# sourceMappingURL=polymarket-driver.js.map