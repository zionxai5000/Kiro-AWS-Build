...
const REQUIRED_DRIVER_PROPERTIES = ['name', 'version'] as const;
...
export class DriverRegistry {
  private readonly drivers = new Map<string, RegistryEntry>();
   ...
   */
  async registerDriver<TConfig = unknown>(
    driver: Driver<TConfig>,
    options: RegisterDriverOptions<TConfig> = {},
  ): Promise<void> {
  ...
  getDriver<TConfig = unknown>(name: string): Driver<TConfig> {
    ...
    if (entry.status !== 'ready') {
      throw new Error(
        `Driver "${name}" is not ready (current status: ${entry.status})`,
      );
    }
    return entry.driver as Driver<TConfig>;
  }
  ...
  listDrivers(): RegisteredDriverInfo[] {
    const result: RegisteredDriverInfo[] = [];
   ...
   */
  private validateDriver(driver: unknown): asserts driver is Driver {
    ...
    for (const method of REQUIRED_DRIVER_METHODS) {
      if (
        !(method in driver) ||
        typeof (driver as Record<string, unknown>)[method] !== 'function'
      ) {
    ...
    if (missing.length > 0) {
      throw new Error(
        `Driver is missing required members: ${missing.join(', ')}`,
      );
    }
  }
}
