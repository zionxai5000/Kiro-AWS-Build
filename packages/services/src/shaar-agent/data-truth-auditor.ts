...
export interface DataTruthReport {
  pageUrl: string;
  timestamp: string;
  overallTruthScore: number; // 0-100 (higher = more real data)
  totalIssues: number;
...
export class DataTruthAuditor {
  /**
   * Audit a page for data truth.
   */
  audit(observation: PageObservation): DataTruthReport {
    const issues: DataTruthIssue[] = [];
    const metrics: MetricAudit[] = [];
  ...
  private detectMockData(observation: PageObservation): DataTruthIssue[] {
    const issues: DataTruthIssue[] = [];
  ...
  private detectPlaceholders(observation: PageObservation): DataTruthIssue[] {
    const issues: DataTruthIssue[] = [];
    const { html } = observation;
    ...
    for (const { pattern, name } of placeholderPatterns) {
      ...
      if (matches && matches.length > 0) {
        ...
        if (name === 'zero metric value' && matches.length < 3) continue;
  ...
  private detectStaleData(observation: PageObservation): DataTruthIssue[] {
    const issues: DataTruthIssue[] = [];
    const { html } = observation;
  ...
  private detectDisconnectedData(observation: PageObservation): DataTruthIssue[] {
    const issues: DataTruthIssue[] = [];
    const { html } = observation;
  ...
  private detectHardcodedValues(observation: PageObservation): DataTruthIssue[] {
    const issues: DataTruthIssue[] = [];
    const { html } = observation;
  ...
  private detectTestData(observation: PageObservation): DataTruthIssue[] {
    const issues: DataTruthIssue[] = [];
    const { html } = observation;
  ...
  private auditMetrics(observation: PageObservation): MetricAudit[] {
    const metrics: MetricAudit[] = [];
    const { html } = observation;
  ...
  private auditSingleMetric(label: string, value: string): MetricAudit {
