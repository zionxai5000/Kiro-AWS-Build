...
$buildId = "b2ab2644-dea7-4e2c-ada0-db2afe0b8b04"
...
while (((Get-Date) - $start).TotalMinutes -lt $maxMinutes) {
  ...
  try {
    ...
    Write-Output "$(Get-Date -Format 'HH:mm:ss') [+${elapsed}s] Build #23: $($build.status)"
    if ($build.status -in 'FINISHED','ERRORED','CANCELED') {
      ...
      $build | ConvertTo-Json -Depth 30 | Set-Content "C:\Users\eftn\Desktop\githubcopy\Kiro-AWS-Build\scripts\build23-meta.json"
      exit 0
    }
