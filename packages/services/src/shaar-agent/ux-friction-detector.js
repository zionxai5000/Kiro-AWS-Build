...
exports.UXFrictionDetector = void 0;
class UXFrictionDetector {
    /**
    ...
    // Friction Detectors
    // -------------------------------------------------------------------------
    detectMissingLabels(observation) {
        const issues = [];
    ...
    detectDeadEnds(observation) {
        ...
        for (const section of sections) {
            ...
            if (section.text && section.text.length > 50) {
                const sectionHtml = html.substring(html.indexOf(section.text?.substring(0, 30) || ''), html.indexOf(section.text?.substring(0, 30) || '') + 2000);
    ...
    detectHiddenStatus(observation) {
        const issues = [];
    ...
    detectMissingFeedback(observation) {
        const issues = [];
        const { elements, html } = observation;
        ...
        const asyncButtons = elements.filter(e => e.tag === 'button' &&
        ...
        for (const btn of asyncButtons) {
            ...
            const btnContext = html.substring(Math.max(0, html.indexOf(btnText) - 200), html.indexOf(btnText) + 500);
    ...
    detectUnclearNavigation(observation) {
        const issues = [];
        const { elements, html } = observation;
        // Check if there's a clear active state in navigation
        const navElements = elements.filter(e => e.tag === 'nav' || e.tag === 'a');
        const hasActiveIndicator = navElements.some(e => e.classes.some(c => c.includes('active') || c.includes('current') || c.includes('selected')));
    ...
    detectCognitiveOverload(observation) {
        const issues = [];
        const { elements, html } = observation;
        // Count interactive elements — too many = cognitive overload
        const interactiveCount = elements.filter(e => e.tag === 'button' || e.tag === 'input' || e.tag === 'a').length;
    ...
    detectPoorHierarchy(observation) {
        const issues = [];
        ...
        const buttons = elements.filter(e => e.tag === 'button');
        const primaryButtons = buttons.filter(e => e.classes.some(c => c.includes('primary') || c.includes('cta') || c.includes('main')));
    ...
    detectAccessibilityIssues(observation) {
        const issues = [];
        const { elements, html } = observation;
        // Check for role attributes on interactive elements
        const interactiveWithoutRole = elements.filter(e => (e.tag === 'button' || e.tag === 'input') && !e.role && !e.ariaLabel);
    ...
    detectErrorHandling(observation) {
        const issues = [];
    ...
    // Scoring
    // -------------------------------------------------------------------------
    calculateCognitiveLoad(observation) {
        const { elements, html } = observation;
        let score = 0;
    ...
    calculateHierarchyScore(observation) {
        const { elements } = observation;
        let score = 100;
        ...
        const buttons = elements.filter(e => e.tag === 'button');
        const hasPrimary = buttons.some(e => e.classes.some(c => c.includes('primary') || c.includes('cta')));
        if (buttons.length > 0 && !hasPrimary)
            score -= 20;
...
exports.UXFrictionDetector = UXFrictionDetector;
//# sourceMappingURL=ux-friction-detector.js.map