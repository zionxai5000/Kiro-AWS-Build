...
$buildId = "10c31d08-cac1-4cb7-ba4a-51aa69b37282"
...
while (((Get-Date) - $start).TotalMinutes -lt $maxMinutes) {
  ...
  try {
    ...
    Write-Output "$(Get-Date -Format 'HH:mm:ss') [+${elapsed}s] Build #21: $($build.status)"
    if ($build.status -in 'FINISHED','ERRORED','CANCELED') {
      ...
      $build | ConvertTo-Json -Depth 30 | Set-Content "C:\Users\eftn\Desktop\githubcopy\Kiro-AWS-Build\scripts\build21-meta.json"
      exit 0
    }
