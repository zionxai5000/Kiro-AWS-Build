...
exports.AppStoreConnectDriver = exports.APP_STORE_ERROR_CODES = exports.APP_STORE_REJECTION_REASONS = void 0;
const driver_js_1 = require("../base/driver.js");
exports.APP_STORE_REJECTION_REASONS = {
    'GUIDELINE_2_1': {
        code: 'GUIDELINE_2_1',
...
// ---------------------------------------------------------------------------
class AppStoreConnectDriver extends driver_js_1.BaseDriver {
    credentialManager;
    ...
    async doExecute(operation) {
        ...
        if (!this._driverConfig) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.UNAUTHORIZED, 'Driver is not connected', false);
        }
        switch (operation.type) {
            ...
            default:
                return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.UNSUPPORTED_OPERATION, `Unsupported operation type: ${operation.type}`, false);
        }
    }
    ...
    async handleCreateApp(operation, operationId) {
        const { bundleId, name, primaryLocale, sku } = operation.params;
        if (!bundleId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'bundleId is required for createApp', false);
        }
        if (!name) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'name is required for createApp', false);
        }
        if (!primaryLocale) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'primaryLocale is required for createApp', false);
        }
        if (!sku) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'sku is required for createApp', false);
        }
        const result = {
            success: true,
            data: {
    ...
    async handleUploadBuild(operation, operationId) {
        const { appId, buildPath, version, buildNumber } = operation.params;
        if (!appId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'appId is required for uploadBuild', false);
        }
        if (!buildPath) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'buildPath is required for uploadBuild', false);
        }
        if (!version) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'version is required for uploadBuild', false);
        }
        if (!buildNumber) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'buildNumber is required for uploadBuild', false);
        }
        const result = {
            success: true,
            data: {
    ...
    async handleSubmitForReview(operation, operationId) {
        const { appId, versionId, buildId } = operation.params;
        if (!appId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'appId is required for submitForReview', false);
        }
        if (!versionId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'versionId is required for submitForReview', false);
        }
        if (!buildId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'buildId is required for submitForReview', false);
        }
        const result = {
            success: true,
            data: {
    ...
    async handleCheckReviewStatus(operation, operationId) {
        const { appId, versionId } = operation.params;
        if (!appId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'appId is required for checkReviewStatus', false);
        }
        if (!versionId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'versionId is required for checkReviewStatus', false);
        }
        const result = {
            success: true,
            data: {
    ...
    async handleUpdateMetadata(operation, operationId) {
        const { appId, versionId, metadata } = operation.params;
        if (!appId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'appId is required for updateMetadata', false);
        }
        if (!versionId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'versionId is required for updateMetadata', false);
        }
        if (!metadata || typeof metadata !== 'object') {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'metadata object is required for updateMetadata', false);
        }
        const result = {
            success: true,
            data: {
    ...
    async handleUploadScreenshots(operation, operationId) {
        const { appId, versionId, locale, deviceType, screenshots } = operation.params;
        if (!appId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'appId is required for uploadScreenshots', false);
        }
        if (!versionId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'versionId is required for uploadScreenshots', false);
        }
        if (!locale) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'locale is required for uploadScreenshots', false);
        }
        if (!deviceType) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'deviceType is required for uploadScreenshots', false);
        }
        if (!screenshots || !Array.isArray(screenshots) || screenshots.length === 0) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'screenshots array is required for uploadScreenshots', false);
        }
        const result = {
            success: true,
            data: {
    ...
    async handleManageSubscriptions(operation, operationId) {
        const { appId, action: subAction, subscriptionGroupId, subscriptionId, details } = operation.params;
        if (!appId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'appId is required for manageSubscriptions', false);
        }
        if (!subAction) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'action is required for manageSubscriptions', false);
        }
        const validActions = ['create', 'update', 'delete', 'list'];
        if (!validActions.includes(subAction)) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, `Invalid subscription action: ${subAction}. Must be one of: ${validActions.join(', ')}`, false);
        }
        const result = {
            success: true,
            data: {
    ...
    async handleGetAppAnalytics(operation, operationId) {
        const { appId, startDate, endDate, metrics } = operation.params;
        if (!appId) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'appId is required for getAppAnalytics', false);
        }
        if (!startDate) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'startDate is required for getAppAnalytics', false);
        }
        if (!endDate) {
            return this.errorResult(operationId, exports.APP_STORE_ERROR_CODES.INVALID_PARAMS, 'endDate is required for getAppAnalytics', false);
        }
    ...
    parseRejection(rejectionCodes) {
        return rejectionCodes
            .map((code) => exports.APP_STORE_REJECTION_REASONS[code])
     ...
     */
    createRejectionResult(operationId, appId, versionId, rejectionCodes) {
        const reasons = this.parseRejection(rejectionCodes);
        return {
            success: false,
            error: {
                code: exports.APP_STORE_ERROR_CODES.APP_REJECTED,
...
exports.AppStoreConnectDriver = AppStoreConnectDriver;
//# sourceMappingURL=appstore-connect-driver.js.map