...
exports.ParallelSchedulerImpl = void 0;
// ---------------------------------------------------------------------------
// Implementation
// ---------------------------------------------------------------------------
/**
...
class ParallelSchedulerImpl {
     ...
     */
    async dispatchBatch(tasks, dagId) {
     ...
     */
    assignRoundRobin(agentId, limit) {
     ...
     */
    assignLeastLoaded(agentId, limit) {
        // Count tasks per slot for this agent
        const slotCounts = new Array(limit).fill(0);
        for (const active of this.activeTasks.values()) {
     ...
     */
    async checkBudget(agentId, task) {
        if (!this.config.budgetChecker) {
            return { allowed: true };
        }
...
exports.ParallelSchedulerImpl = ParallelSchedulerImpl;
